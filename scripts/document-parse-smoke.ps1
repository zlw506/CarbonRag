param(
  [switch]$RequireDocling = $false
)

$ErrorActionPreference = "Stop"
$repoRoot = Split-Path -Parent $PSScriptRoot
$python = Join-Path $repoRoot "backend/.conda/python.exe"
if (-not (Test-Path $python)) {
  $python = "python"
}
Set-Location $repoRoot

Write-Host "==> Running document parse smoke"
$script = @'
...
if ($RequireDocling) {
  Write-Host "==> Checking Docling import"
