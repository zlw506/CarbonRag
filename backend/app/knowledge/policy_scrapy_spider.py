...
class CarbonRagPolicySpider(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.max_pages = max(1, int(max_pages or 20))
...
def build_scrapy_settings(request: dict[str, Any]) -> dict[str, Any]:
    ...
    download_delay = float(request.get("download_delay_seconds") or 2.0)
    return {
        "AUTOTHROTTLE_ENABLED": True,
        "AUTOTHROTTLE_START_DELAY": max(0.1, download_delay),
        "AUTOTHROTTLE_MAX_DELAY": max(10.0, download_delay * 5),
        "CLOSESPIDER_PAGECOUNT": int(request.get("max_pages") or 20),
...
def build_spider_kwargs(
    ...
    *,
    ...
    return {
        ...
        "max_pages": str(int(request.get("max_pages") or 20)),
...
def _should_capture_document(*, url: str, title: Any, content_type: str, depth: int, is_seed_url: bool) -> bool:
    ...
    if content_type != "text/html":
        return True
    if is_seed_url and not _looks_like_policy_title(title):
        return False
    return _looks_like_policy_url(url) or _looks_like_policy_title(title)
