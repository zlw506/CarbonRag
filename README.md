...
当前状态：第 0.1.3B 轮静态快照研究中。

## 项目定位
CarbonRag 是一个面向中小企业的“双碳”政策与企业应用智能问答 MVP，目标是围绕政策理解、企业样例接入、基础碳核算和报告生成形成首条可演示闭环。

## 当前阶段
- 阶段名称：静态快照研究轮（Static Snapshot Study）
- 当前研究对象：`claw-code` 本地静态快照
- 当前策略：统一使用第三方云端 API，保留后续替换为本地或私有模型的接口能力
- 当前默认模型：`gpt-5.4`
- 当前原则：研究外部 AI runtime 分层，但不把学习样本接入 CarbonRag 运行时
...
## 仓库说明
本仓库当前保留 v0.0.2 的可运行工程壳，并进入 v0.1.3B 静态快照研究轮。当前新增内容以第三方架构学习文档为主，不改变 CarbonRag 的前后端运行时边界。后续开发必须基于 `dev` 或 `feature/*` 分支推进，并持续遵守文档中定义的边界与提交纪律。
